				...
				<div class="panel-heading">Data dosen</div>
					...
					<form action="" method="POST">
						...
						<div class="form-group">
							...
							<h5 style="padding-left: 5px;font-style: italic;">Jika Ingin Mengganti periode setelah proses HITUNG, Mohon Untuk Refresh Halaman kembali.</h5>
				  </div>
				  <input type="submit" name="pilih_periode" class="btn btn-primary" value="Pilih">
				</form>    			
			</div>
			...
			<?php
                                if (isset($_POST['pilih_periode'])) {
                                    ...
                                    $nama_periode = $rowper['periode'];
                                        ...
                                        ?>
										<br><div class="col-lg-8 col-lg-offset-2">
								<h4 style="font-style: oblique;font-weight: bold;color:black;">Periode terpilih : <?php echo $nama_periode; ?></h4></div>
								
								
                                        
                                        <script type="text/javascript">
											//  var yeet = $nama_periode;
                                            alert("Periode Berhasil di Ubah");
...
<?php
	$ps = mysqli_query($koneksi,"select * from dosen");
	while ($rps=mysqli_fetch_array($ps)){
		...
		$upd = mysqli_query($koneksi,"update dosen set vektor_s = '$vkt_s' where nidn = '$rps[nidn]'");
	}

	$v = mysqli_query($koneksi,"select sum(vektor_s) as all_vk from dosen");
	$vk = mysqli_fetch_array($v);
	$all_vk = $vk['all_vk'];
	
	$ps = mysqli_query($koneksi,"select nidn, vektor_s from dosen");
	while ($rps=mysqli_fetch_array($ps)){
		$vk_v = $rps['vektor_s']/$all_vk;
		$up_v = mysqli_query($koneksi,"update dosen set vektor_v = '$vk_v' where nidn='$rps[nidn]'");
	}

	$period = mysqli_query($koneksi,"select * from dosen order by vektor_v desc LIMIT 10");
						    		
                                    while ($data=mysqli_fetch_array($period)) {
										//$curYear = date('Y'); 
										$periode = $_SESSION['periode'] ;
                                        $tambah = mysqli_query($koneksi, "INSERT INTO periode_pemenang (id, id_peserta_dosen, nidn, id_periode) VALUES ('', $data[id_peserta_dosen], $data[nidn], '$periode')");
                                    }
									$sqls = "SELECT periode from periode WHERE id_periode = $_SESSION[periode] ";
                                    $queryperiode = mysqli_query($koneksi, $sqls);
                                    $rowper = mysqli_fetch_array($queryperiode);
			...
			<div class="col-md-12">
				...
				<div class="panel-heading">Hasil Seleksi</div>
					...
					<div class="col-lg-8 col-lg-offset-2" align='center'>
								...
								<h3 style="font-style: oblique;font-weight: bold; color:black;">Periode terpilih : <?php echo $nama_periode; ?></h3></div>
					<table class="table table-striped table-bordered data">
							<thead>
								<tr>
									<th>Ranking</th>
						    ...
						    <tbody>
						    	<?php
						    		$rk = 1;
						    		$pr = mysqli_query($koneksi,"select nama, vektor_s, vektor_v from dosen order by vektor_v desc");
